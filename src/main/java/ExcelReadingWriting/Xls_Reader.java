...
public class Xls_Reader {
	...
	public boolean checkColumnWithNameExistsOrNot(String sheetName, String colName) {
		boolean check= false;
		int index = workbook.getSheetIndex(sheetName);
	    ...
	    for (int i = 0; i < getColumnCount(sheetName); i++) {
	    	...
	    	//System.out.println(getCellData(sheetName, i, 1));
	    	if (getCellData(sheetName, i, 1).equalsIgnoreCase(colName)) {
	    		System.out.println("Existing of Column name at column number "+i);
				return true;
			}
	    }
		return check;
