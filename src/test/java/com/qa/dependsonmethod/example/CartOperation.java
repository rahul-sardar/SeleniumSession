...
public class CartOperation {
    ...
    @Test(dependsOnMethods = {"login", "testAddProductToCart"})
...
/*
 * Depends on methods for the test will skip the test case if the dependent
 * method fail. Now what will happen if we use priority and dependsonmethod
 * together, suppose we give priority=1 to 'dependsonmethod' ->No, Nothing will
 * happen. Priority will be given to dependsOnMethods, even though you give
 * priority it will be overrided by dependsOnMethods
 * 
 * Now what if we give ignore tag on login method,
 * --> TestNG will give exception- It will say other test are dependent don't use ignore
 * TestNGException we will get.
 * 
 * What if there is no method on which you are depending and you mention it under dependsOnMethods, then same TestNGException will be shown.
 */
